...
**Reason**: the requirement's whole point was that the directory is populated
*exclusively* by data migrations and that **no user-facing flow creates, edits or
deletes clients**. This change introduces exactly that flow, so the concept
inverts rather than shifting; amending it in place would leave a requirement
whose title and scenarios assert the opposite of the behavior. Replaced by
*The client directory is managed from the back office*, which keeps everything
the old requirement got right — the stable identifier, the slug as natural key,
idempotent migration upserts, and retirement by inactivity rather than deletion.
