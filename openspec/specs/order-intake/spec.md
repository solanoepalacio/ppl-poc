...
- **AND** transitions that order's status to `issued`
...
- **THEN** the system rejects the submission and the order remains `pending`
...
- **THEN** the system transitions the order bound to the token to `denied`
