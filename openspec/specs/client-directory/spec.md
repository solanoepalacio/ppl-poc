...
Defines the fixed directory of clients that orders are attached to: a preset list of clients (display name + normalized slug + active flag) loaded via Prisma data migrations with no management UI, and the back-office endpoint that exposes the active clients an order can be attached to.
